...
기존 `README_*.md` 파일을 보존해야 한다는 전제를 버린다. Git 히스토리가 있으므로 현재 기준으로 쓰지 않는 초안 문서는 repository 안에 계속 둘 필요가 없다.
...
아래 문서는 이번 clean slate 설계로 대체된다.

- `docs/superpowers/specs/2026-05-22-readme-ai-docs-cleanup-design.md`
- `docs/superpowers/plans/2026-05-22-readme-ai-docs-cleanup.md`
...
- `rg -n "README_CODEX|README_ARCHITECTURE|README_PRODUCT|README_JUNIE|readme-ai-docs-cleanup"` 결과가 0건인지 확인한다.
...
- GitHub 이슈 템플릿의 외부 링크가 깨질 수 있다.
  - 대응: `README_CODEX.md`, `README_ARCHITECTURE.md` 링크를 각각 `docs/development.md`, `docs/architecture.md`로 갱신한다.

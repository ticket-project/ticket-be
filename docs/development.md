...
- 대기열: Redis 기반 queue ticket, 토큰, 입장 상태 관리
...
대기열은 회차 진입 제어와 좌석/hold API 접근 제어 앞단에 위치한다.

주요 개념:

- queue ticket
- queue token
- waiting/admitted/expired 상태
- Redis TTL 기반 토큰 수명
- 입장 허용 수 제어

주요 위치:

- `core/core-api/src/main/java/com/ticket/core/api/controller/QueueController.java`
- `core/core-api/src/main/java/com/ticket/core/config/QueueAdmissionInterceptor.java`
- `core/core-domain/src/main/java/com/ticket/core/domain/queue`
- `core/core-infra/src/main/java/com/ticket/core/infra/queue`
- `core/core-infra/src/main/java/com/ticket/core/infra/redis/QueueTokenExpirationHandler.java`
...
- Redis 기반 대기열 상태
- 토큰, position, admitted 상태를 관리
- 좌석 조회와 hold 진입 전 제어에 사용

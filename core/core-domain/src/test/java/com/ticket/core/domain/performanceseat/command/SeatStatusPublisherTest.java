...
class SeatStatusPublisherTest {
    ...
    void 좌석_선점상태는_좌석별_HELD_이벤트로_발행한다() {
        ...
        verify(seatEventPort).publish(10L, 1L, HELD);
        verify(seatEventPort).publish(10L, 2L, HELD);
    ...
    void 좌석_해제상태는_좌석별_RELEASED_이벤트로_발행한다() {
        ...
        verify(seatEventPort).publish(10L, 3L, RELEASED);
        verify(seatEventPort).publish(10L, 4L, RELEASED);
        verify(seatEventPort, times(2)).publish(org.mockito.ArgumentMatchers.anyLong(), org.mockito.ArgumentMatchers.anyLong(), org.mockito.ArgumentMatchers.any());
    }
}

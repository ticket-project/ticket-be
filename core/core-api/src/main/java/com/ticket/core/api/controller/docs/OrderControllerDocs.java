...
public interface OrderControllerDocs {
    ...
    @Operation(
            summary = "주문 시작",
            description = """
                    요청한 좌석을 선점하고 결제 진입용 PENDING 주문을 시작합니다.
                    동일 회원은 같은 회차에 PENDING 주문을 1건만 가질 수 있습니다.
    ...
    @ApiResponses(value = {
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "409",
                    description = "이미 선점된 좌석이 있거나 진행 중인 PENDING 주문이 존재함",
                    content = @Content(schema = @Schema(implementation = com.ticket.core.support.response.ApiResponse.class))
            )
    })
    ResponseEntity<ApiResponse<StartOrderUseCase.Output>> startOrder(
            StartOrderRequest request,
            @Parameter(hidden = true) MemberPrincipal memberPrincipal
    );

    @Operation(summary = "주문 조회", description = "주문/결제 화면 진입 때 호출합니다. 만료된 주문은 EXPIRED 상태로 표시됩니다.")
    @ApiResponses(value = {

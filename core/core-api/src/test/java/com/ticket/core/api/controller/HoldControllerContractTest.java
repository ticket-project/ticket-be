...
class HoldControllerContractTest {
    ...
    @Test
    void HOLD_생성_성공시_201_헤더와_응답바디_계약을_유지한다() throws Exception {
        when(startOrderUseCase.execute(new StartOrderUseCase.Input(10L, List.of(7L, 3L), 100L)))
                .thenReturn(new StartOrderUseCase.Output("ORD-20260320"));
        ...
        mockMvc.perform(post("/api/v1/performances/10/holds")
                ...
                .andExpect(header().string("Location", "/api/v1/orders/ORD-20260320"))
                .andExpect(header().string("X-Order-Key", "ORD-20260320"))
                .andExpect(jsonPath("$.result").value("SUCCESS"))
                .andExpect(jsonPath("$.data.orderKey").value("ORD-20260320"))

...
기능 흐름이 Redis 구현체를 직접 참조하지 않도록, `auth`는 application-facing port를 먼저 둔다.
...
- 실제 Redis/HTTP 구현은 `auth.infra.*` 패키지에서 port를 구현한다.
...
- `domain.auth.command`와 `domain.auth.oauth2`는 `domain.auth.infra` 구현체에 직접 의존하지 않는다.
- `core-api`의 `config.security`는 `domain.auth.infra` 구현체에 직접 의존하지 않는다.
